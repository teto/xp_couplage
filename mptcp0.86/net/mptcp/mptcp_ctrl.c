...
int mptcp_alloc_mpcb(struct sock *meta_sk, __u64 remote_key, u32 window)
{
	...
	mpcb->number_of_remote_rlocs = 1;
	mpcb->number_of_local_rlocs = 1;
	
	// mpcb->number_of_subflows_to_create = 1;


	/* Initialize workqueue-struct */
	INIT_WORK(&mpcb->path_discovery_work, mptcp_path_discovery_worker);
	
	/** are these launched only when previous worker returns ? looks like it
	 
	 ==================================
	 NOTE replaced original mptcp_create_subflow_worker by 
	 mptcp_create_subflow_worker_homemade
	 ==================================
	 
	 **/
	// INIT_WORK(&mpcb->subflow_work, mptcp_create_subflow_worker);
	INIT_WORK(&mpcb->subflow_work, mptcp_create_subflow_worker_homemade );


	INIT_DELAYED_WORK(&mpcb->subflow_retry_work, mptcp_retry_subflow_worker);
	
	 /** handles netdev up and down **/
...
void mptcp_update_metasocket(struct sock *sk, struct sock *meta_sk)
{
	...
	// Queue work since we know addresses are set
	if (!work_pending(&mpcb->path_discovery_work)) {
		
		// sock_hold(meta_sk); // useful ?
		queue_work(mptcp_wq, &mpcb->path_discovery_work);
	}


	/**
	TODO tester en la supprimant
	**/
