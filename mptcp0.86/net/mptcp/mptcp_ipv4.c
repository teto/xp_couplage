...
static int mptcp_get_port_modulo_n(int desired_port_rest_after_modulo,int modulo)
{
	...
	mptcp_debug("Looking for a port number with rest after modulo %d of %d (rest) ? ... ", modulo,desired_port_rest_after_modulo);
			   ...
			   struct mptcp_rem4 *rem)
{
	...
	/** TODO here we will be able to choose port **/
	loc_in.sin_port = 0;
	if (rem->port)
